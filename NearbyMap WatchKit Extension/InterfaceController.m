...
- (void)refreshMenu {
	...
	NSString *markerFixTitle = (isMarkerFix==YES?@"マーカー移動":@"マーカー固定");
	
	[self addMenuItemWithItemIcon:WKMenuItemIconPause   title:mapFixTitle action:@selector(fixMap)];
	[self addMenuItemWithItemIcon:WKMenuItemIconBlock   title:markerFixTitle action:@selector(fixMarker)];
	[self addMenuItemWithItemIcon:WKMenuItemIconDecline title:markerFixTitle action:nil];
#endif
}
